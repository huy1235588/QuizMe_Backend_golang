...
import (
	...
	"github.com/huy/quizme-backend/internal/features/user/service"
	"github.com/huy/quizme-backend/internal/infra/middleware"
	"github.com/huy/quizme-backend/internal/infra/storage"
)
...
func (h *UserHandler) GetPagedUsers(c *gin.Context) {
	...
	users, total, err := h.userService.GetUsersPaged(page, pageSize, search, sortBy, sortDir)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.Error("Failed to get users"))
		return
	}

	pageResponse := response.PageResponse[[]userdto.UserResponse]{
		Content:       users,
		Page:          page,
		Size:          pageSize,
		TotalElements: total,
...
func (h *UserHandler) UploadAvatar(c *gin.Context) {
	...
	// Use current user ID as profile ID
