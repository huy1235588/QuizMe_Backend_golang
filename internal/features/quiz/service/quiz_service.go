...
func (s *quizService) GetAllQuizzes() ([]*quizdto.QuizResponse, error) {
	quizzes, err := s.quizRepo.FindAll()
	if err != nil {
		return nil, err
	}
	return quizdto.FromQuizzes(quizzes), nil
}

func (s *quizService) GetPublicQuizzes() ([]*quizdto.QuizResponse, error) {
...
func (s *quizService) GetQuizByID(id uint) (*quizdto.QuizResponse, error) {
	...
	return quizdto.FromQuiz(quiz), nil
}

func (s *quizService) GetQuizWithQuestions(id uint) (*quizdto.QuizResponse, []quizdto.QuestionResponse, error) {
	...
	quizResponse := quizdto.FromQuiz(quiz)
	questionResponses := make([]quizdto.QuestionResponse, len(quiz.Questions))
	for i, q := range quiz.Questions {
		questionResponses[i] = *quizdto.FromQuestion(&q)
	}

	return quizResponse, questionResponses, nil
}

func (s *quizService) GetQuizzesByDifficulty(difficulty enums.Difficulty) ([]*quizdto.QuizResponse, error) {
...
) ([]*quizdto.QuizResponse, int64, error) {
	quizzes, total, err := s.quizRepo.FindWithFilters(categoryID, difficulty, isPublic, search, page, pageSize, sortBy, sortDir)
	if err != nil {
		return nil, 0, err
	}
	return quizdto.FromQuizzes(quizzes), total, nil
}

func (s *quizService) CreateQuiz(creatorID uint, req *quizdto.QuizRequest) (*quizdto.QuizResponse, error) {
	isPublic := true
	if req.IsPublic != nil {
...
func (s *quizService) UpdateQuiz(id, userID uint, req *quizdto.QuizRequest) (*quizdto.QuizResponse, error) {
	quiz, err := s.quizRepo.FindByID(id)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrQuizNotFound
